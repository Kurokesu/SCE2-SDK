...
def split_preset_values(str_values, value_count=6):
...
class MyWindowClass(QtWidgets.QMainWindow, gui.Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.btn_f1.clicked.connect(self.btn_f1_clicked)
        self.btn_f2.clicked.connect(self.btn_f2_clicked)
        self.btn_f3.clicked.connect(self.btn_f3_clicked)
        ...
        self.btn_iris_mid.clicked.connect(self.btn_iris_mid_clicked)
        ...
        self.presets = preset_widget.Preset(self.horizontalLayout, 6)
        self.presets.clicked_set.connect(self.preset_set)
        self.presets.clicked_go.connect(self.preset_go)
        
        self.pos = pos_widget.Pos(self.verticalLayout, 6)
        ...
        self.group_guided_zoom1.hide()
        self.group_guided_focus1.hide()
        self.group_keypoints_2.hide()
        self.group_pi.hide()
        self.label_cal_note.hide()
    ...
    def preset_go(self, nr, values):
        ...
        if self.config["lens"][self.lens_name]["motor"]["function"]["axis_a"]:
            cmd += " A"
            cmd += str(values[3])
        if self.config["lens"][self.lens_name]["motor"]["function"]["axis_b"]:
            cmd += " B"
            cmd += str(values[4])
        if self.config["lens"][self.lens_name]["motor"]["function"]["axis_c"]:
            cmd += " C"
            cmd += str(values[5])
        cmd += " F"
    ...
    def preset_set(self, nr):
        #print("set", nr)
        self.presets.set_values(nr, ch0=self.status.pos_x, ch1=self.status.pos_y, ch2=self.status.pos_z, ch3=self.status.pos_a, ch4=self.status.pos_b, ch5=self.status.pos_c)
    ...
    def comboBox_keypoints_changed(self, value):
        if len(value)>0:
            ...
            #self.label_dk_b.setText(str(p[4]))
            #self.label_dk_c.setText(str(p[5]))
    ...
    def btn_all_seek_clicked(self):
        ...
        if self.lens_name == "DB001":
            ...
            # offset axies after homing pullback to 0.5
            #cmd = "G10 L20 P1 X0.5 Y0.5 Z0.5 A0.5 B0.5 C0.5"
            #self.hw.send(cmd+"\n")
            
            # move to pos0
            #cmd = "G0 X0 Y0 Z0 A0 B0 C0"
            #self.hw.send(cmd+"\n")


            # G10 L20 P1 X0 Y0 Z0 A0 B0 C0
            # G92 x-0.5 y-0.5 z-0.5 a-0.5 b-0.5 c-0.5
            # G92.1 x0.5 y0.5 z0.5 a0.5 b0.5 c0.5
            # G0 x0 y0 z0 a0 b0 c0
            # G0 x0.5 y0.5 z0.5 a0.5 b0.5 c0.5
            # G54 x-0.5 y-0.5 z-0.5 a-0.5 b-0.5 c-0.5
    ...
    def btn_f1_clicked(self):
        # visible
        cmd =  "G90 G1 Z0 F1000"
        self.hw.send(cmd+"\n")

    def btn_f2_clicked(self):
        # NIR
        cmd =  "G90 G1 Z-19 F1000"
        self.hw.send(cmd+"\n")

    def btn_f3_clicked(self):
        # FULL
        cmd =  "G90 G1 Z21 F1000"
        self.hw.send(cmd+"\n")
    ...
    def btn_iris_mid_clicked(self):
        cmd =  "G90 G1 B2.6 F1000"
        self.hw.send(cmd+"\n")

    def btn_iris_off_clicked(self):
        cmd =  "G90 G1 B5.2 F1000"
        self.hw.send(cmd+"\n")
    ...
    def serVersion(self, text):
        ...
        for i in id_strings:
            ...
            if lens_detected:
                ...
                preset = split_preset_values(self.config["lens"][self.lens_name]["preset"]["p1"], value_count=6)
                self.presets.set_values(0, ch0=preset[0], ch1=preset[1], ch2=preset[2], ch3=preset[3], ch4=preset[4], ch5=preset[5])

                preset = split_preset_values(self.config["lens"][self.lens_name]["preset"]["p2"], value_count=6)
                self.presets.set_values(1, ch0=preset[0], ch1=preset[1], ch2=preset[2], ch3=preset[3], ch4=preset[4], ch5=preset[5])

                preset = split_preset_values(self.config["lens"][self.lens_name]["preset"]["p3"], value_count=6)
                self.presets.set_values(2, ch0=preset[0], ch1=preset[1], ch2=preset[2], ch3=preset[3], ch4=preset[4], ch5=preset[5])

                preset = split_preset_values(self.config["lens"][self.lens_name]["preset"]["p4"], value_count=6)
                self.presets.set_values(3, ch0=preset[0], ch1=preset[1], ch2=preset[2], ch3=preset[3], ch4=preset[4], ch5=preset[5])
                
                preset = split_preset_values(self.config["lens"][self.lens_name]["preset"]["p5"], value_count=6)
                self.presets.set_values(4, ch0=preset[0], ch1=preset[1], ch2=preset[2], ch3=preset[3], ch4=preset[4], ch5=preset[5])

                preset = split_preset_values(self.config["lens"][self.lens_name]["preset"]["p6"], value_count=6)
                self.presets.set_values(5, ch0=preset[0], ch1=preset[1], ch2=preset[2], ch3=preset[3], ch4=preset[4], ch5=preset[5])
                ...
                if self.config["lens"][self.lens_name]["motor"]["function"]["axis_b"]:
                    self.pos.set_name(4, "B axis / " + self.config["lens"][self.lens_name]["motor"]["function"]["axis_b"])
                if self.config["lens"][self.lens_name]["motor"]["function"]["axis_c"]:
                    self.pos.set_name(5, "C axis / " + self.config["lens"][self.lens_name]["motor"]["function"]["axis_c"])
                ...
                if self.config["lens"][self.lens_name]["motor"]["curves2"]:
                    #print(self.config["lens"][self.lens_name]["motor"]["curves2"])
                    lens_yaml = self.config["lens"][self.lens_name]["motor"]["curves2"]["inf"]
                    data_x = lens_yaml["x"].copy()
                    data_y = lens_yaml["y"].copy()
                    data_a = lens_yaml["a"].copy()
                    data_c = lens_yaml["c"].copy()

                    for i in range(len(data_x)):
                        data_x[i] = data_x[i]-0.5

                    for i in range(len(data_y)):
                        data_y[i] = data_y[i]-0.5

                    for i in range(len(data_a)):
                        data_a[i] = data_a[i]-0.5

                    for i in range(len(data_c)):
                        data_c[i] = data_c[i] / 5 - 0.5



                    pen2 = pg.mkPen(color="red", style=QtCore.Qt.SolidLine)
                    im_x = self.plot.plot(pen=pen2, name="Focus")
                    im_x.setData(x=data_c, y=data_x)

                    pen2 = pg.mkPen(color="blue", style=QtCore.Qt.SolidLine)
                    im_y = self.plot.plot(pen=pen2, name="Zoom2")
                    im_y.setData(x=data_c, y=data_y)

                    pen2 = pg.mkPen(color="green", style=QtCore.Qt.SolidLine)
                    im_a = self.plot.plot(pen=pen2, name="Zoom3")
                    im_a.setData(x=data_c, y=data_a)
                ...
                self.scatter_x = self.plot.plot(pen=None, symbol='x', symbolPen="red", symbolSize = 15)
                self.scatter_y = self.plot.plot(pen=None, symbol='x', symbolPen="blue", symbolSize = 15)
                self.scatter_a = self.plot.plot(pen=None, symbol='x', symbolPen="green", symbolSize = 15)
                #self.scatter_c = self.plot.plot(pen=None, symbol='c zoom1', symbolPen="gree", symbolSize = 15)

                #data_0 = [0]
                #y_data = [0]
                #self.scatter_focus_zoom.setData([0], y_data)
                #self.scatter_compensate.setData(x_data, y_data)
                #self.scatter_x.setData(data_0, data_0)
                #self.scatter_y.setData(data_0, data_0)
                #self.scatter_a.setData(data_0, data_0)
    ...
    def serFeedback(self, text):
        ...
        for p in txt_list[1:]:
            ...
            if p[0:2] == "Pn":
                ...
                s.limit_b = "B" in temp1
                s.limit_c = "C" in temp1
            ...
            if p[0:4] == "WPos":
                ...
                s.pos_b = float(temp1.split(",")[4])
                s.pos_c = float(temp1.split(",")[5])
        ...
        if self.lens_name:
            ...
            self.scatter_x.setData([s.pos_c], [s.pos_x])
            self.scatter_y.setData([s.pos_c], [s.pos_y])
            self.scatter_a.setData([s.pos_c], [s.pos_a])
            
            #self.scatter_c.setData([s.pos_c], [s.pos_z])
            pass
        ...
        self.pos.set_value(4, s.pos_b, s.limit_b)
        self.pos.set_value(5, s.pos_c, s.limit_c)
    ...
    def closeEvent(self, event):
        ...
        if self.s_status.text() == "Connected":
            ...
            p = str(p[0])+" "+str(p[1])+" "+str(p[2])+" "+str(p[3])+" "+str(p[4])+" "+str(p[5])
            ...
            p = self.presets.get_values(5)
            p = str(p[0])+" "+str(p[1])+" "+str(p[2])+" "+str(p[3])+" "+str(p[4])+" "+str(p[5])
            self.config["lens"][self.lens_name]["preset"]["p6"] = p
